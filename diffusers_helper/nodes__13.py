...
# Added for VAE loading
import comfy.sd
import comfy.vae.vae_model
...
class HyVideoModel(comfy.model_base.BaseModel):
    def __init__(self, *args, **kwargs):
        ...
        self.pipeline = {}
        # Changed this to expect 'device' as a kwargs if passed, otherwise default ComfyUI logic
...
class DownloadAndLoadFramePackModel:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            # New for manual GPU Paralellization
            "gpu_id": ("INT", {"default": 2, "min": 0, "max": torch.cuda.device_count() - 1, "tooltip": "GPU device ID to load model onto"}),
            },
            "optional": {
                "attention_mode": ([
                    "sdpa",
    ...
    def loadmodel(self, model, base_precision, quantization, gpu_id,
        ...
        # Passing the gpu_id to the device
        # device = mm.get_torch_device(f"cuda:{gpu_id}")
        ...
        # Passing the model to the target device directly
        ...
        # Changing the device to the target device
...
class LoadFramePackModel:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "base_precision": (["fp32", "bf16", "fp16"], {"default": "bf16"}),
            "quantization": (['disabled', 'fp8_e4m3fn', 'fp8_e4m3fn_fast', 'fp8_e5m2'], {"default": 'disabled', "tooltip": "optional quantization method"}),
            # New to allow manual GPU Paralellization
            "gpu_id": ("INT", {"default": 2, "min": 0, "max": torch.cuda.device_count() - 1, "tooltip": "GPU device ID to load model onto"}),
            },
            "optional": {
                "attention_mode": ([
    ...
    def loadmodel(self, model, base_precision, quantization, gpu_id,
        ...
        params_to_keep_quant = {"norm", "bias", "time_in", "vector_in", "guidance_in", "txt_in", "img_in"} # Keep this definition
        ...
        # Moving things to the target device
        DynamicSwapInstaller.install_model(transformer, device=target_device)
...
class FramePackSampler:
    ...
    def process(self, model, shift, positive, negative, latent_window_size, use_teacache, total_second_length, teacache_rel_l1_thresh, image_embeds, steps, cfg,
        ...
        # Get the actual device of the transformer
        ...
        # Move to sampler's device
        start_latent = start_latent.to(device)
        if initial_samples is not None:
            initial_samples = initial_samples["samples"] * vae_scaling_factor4
            # Move to sampler's device
        ...
        comfy_model = HyVideoModel(
                ...
                # Pass the actual device to HyVideoModel
                device=device,
            )
        ...
        # IMPORTANT CHANGE HERE
        # Comment out this line as it could interfere with explicit manual placement
        ...
        # This line should ideally not be needed if the model stays on its assigned GPU
        # during its active phase, and offloading is handled by ComfyUI's internal model management.
        # transformer.to(offload_device) # Comment out to rely on explicit placement
        # transformer.to(offload_device)
        # Still good to clear cache
        mm.soft_empty_cache()
...
class FramePackVAELoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "gpu_id": ("INT", {"default": 3, "min": 0, "max": torch.cuda.device_count() - 1, "tooltip": "GPU device ID to load VAE onto"}),
            }
        }
    RETURN_TYPES = ("VAE",)
    FUNCTION = "load_vae"
    ...
    def load_vae(self, vae_name, gpu_id):
        ...
        # vae = comfy.sd.VAE(pretrained_model_name_or_path=None).eval() # Create empty VAE instance
        # vae = comfy.sd.VAE().eval() 
        # Create empty VAE instance
        # vae = comfy.vae.vae_model.VAE(vae_path).eval()
        vae = comfy.vae.vae_model.VAE().eval()
        vae.load_state_dict(vae_sd)
        # vae.load_state_dict(vae_sd)
        return (vae.to(target_device),) 
...
NODE_CLASS_MAPPINGS = {
    ...
    "FramePackVAELoader": FramePackVAELoader,
    }
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "FramePackVAELoader": "FramePack VAELoader", 
    }
