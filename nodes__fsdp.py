...
class DownloadAndLoadFramePackModel:
    ...
    def loadmodel(self, model, base_precision, quantization,
        ...
        # Load the state_dict to CPU first, Which is crucial for FSDP
        ...
        # low_cpu_mem_usage=False, FSDP handles memory distribution
        transformer = HunyuanVideoTransformer3DModelPacked.from_pretrained(model_path, low_cpu_mem_usage=False, 
                                                                           torch_dtype=base_dtype, attention_mode=attention_mode).cpu()
        
        # START FSDP INTEGRATION 
        if dist.is_initialized():
            local_rank = dist.get_rank()
            current_device = torch.device(f"cuda:{local_rank}")

            # Move the model to the current GPU device before FSDP wrapping
            transformer = transformer.to(current_device)

            # Wrapping the transformer model with FSDP
            transformer = FSDP(transformer, 
                               sharding_strategy=ShardingStrategy.FULL_SHARD, 
                               device_id=current_device) 
            print(f"FSDP: HunyuanVideoTransformer3DModelPacked wrapped on GPU {local_rank}.")
        else:
            print("FSDP: Distributed environment not initialized. Running model on single device.")
            transformer = transformer.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        # END FSDP INTEGRATION 
...
class LoadFramePackModel:
    ...
    def loadmodel(self, model, base_precision, quantization,
        ...
        if dist.is_initialized():
            ...
            # Wrap the transformer model with FSDP
            transformer = FSDP(transformer, 
                               sharding_strategy=ShardingStrategy.FULL_SHARD, 
                               device_id=current_device) 
            print(f"FSDP: HunyuanVideoTransformer3DModelPacked wrapped on GPU {local_rank}.")
            ...
            print("FSDP: Distributed environment not initialized. Running model on single device.")
            # Ensure model is on a GPU if available
            transformer = transformer.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        # END FSDP INTEGRATION

        # FSDP has its own memory management for sharded parameters.
        # DynamicSwapInstaller.install_model(transformer, device=device)
