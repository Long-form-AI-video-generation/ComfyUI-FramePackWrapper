...
class DownloadAndLoadFramePackModel:
    ...
    def loadmodel(self, model, base_precision, quantization,
        ...
        #   START CUDA Stream Implementation for Model Loading
        compute_stream = torch.cuda.Stream()
        transfer_stream = torch.cuda.Stream()
        #   END CUDA Stream Implementation for Model Loading 
        ...
        # Load transformer using transfer stream
        ...
        # Ensure transformer is loaded before further processing on compute stream
        ...
        # Install model for dynamic swapping on the compute stream
        with torch.cuda.stream(compute_stream):
            DynamicSwapInstaller.install_model(transformer, device=device)

        if compile_args is not None:
            # Ensure compilation happens after DynamicSwapInstaller and on compute stream
            with torch.cuda.stream(compute_stream):
        ...
        # START CUDA Stream Final Synchronization
        torch.cuda.current_stream().wait_stream(compute_stream)
        torch.cuda.current_stream().wait_stream(transfer_stream)
        # END CUDA Stream Final Synchronization
        return (pipe, )
...
class LoadFramePackModel:
    ...
    def loadmodel(self, model, base_precision, quantization,
        ...
        # Load state_dict on offload_device using the transfer stream
        with torch.cuda.stream(transfer_stream):
            sd = load_torch_file(model_path, device=offload_device, safe_load=True)
            
        # Ensure transfer of state dict is complete before creating transformer on compute stream
        compute_stream.wait_stream(transfer_stream)
        
        with init_empty_weights():
            transformer = HunyuanVideoTransformer3DModelPacked(**config, attention_mode=attention_mode)

        # Use base_dtype as a safe fallback if not explicitly derived from loaded model
        model_weight_dtype = base_dtype 
        ...
        param_count = sum(1 for _ in transformer.named_parameters())
         # Move transformer parameters to device using the transfer stream
        with torch.cuda.stream(transfer_stream):
        ...
        # Ensure all parameter transfers are complete before any further processing (e.g., LoRA, compile)
        compute_stream.wait_stream(transfer_stream)
        ...
        if compile_args is not None:
            ...
            # Full transformer compilation (if desired)   
            #transformer = torch.compile(transformer, fullgraph=compile_args["fullgraph"], dynamic=compile_args["dynamic"], backend=compile_args["backend"], mode=compile_args["mode"])
...
class FramePackSampler:
    ...
    def process(self, model, shift, positive, negative, latent_window_size, use_teacache, total_second_length, teacache_rel_l1_thresh, image_embeds, steps, cfg,
        ...
        compute_stream.wait_stream(transfer_stream)
        # Sampling
        # Sampling operations wrapped in compute_stream
        with torch.cuda.stream(compute_stream):
            ...
            # Transformer is already moved to device by move_model_to_device_with_memory_preservation
